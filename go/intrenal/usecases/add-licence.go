...
func AddLicence(storage *infra.Storage, args []string) []byte {
	if len(args) < 4 {
	...
	if err := storage.AddLicence(t.Name, t.Link); err != nil {
		return FormatJSON(nil, errors.Wrap(err, "error adding type"))
	}
